(function() {
    ...
    var HTTPRequestFactory = module.exports = new Class({
        ...
        , _getAuthentication: function(original) {
            ...
            if (!auth) auth = original.getHeader('x-authorization');
        ...
        /**
         * Get cart-token out of request-cookies
         *
         * @param original
         * @returns String
         * @private
         */
        , _getCartToken: function(original) {
            let cartToken = null;

            if(original.hasHeader('cookie')) {
                original.getHeader('cookie')
                .split(';')
                .some((cookieEntry) => {
                    let keyValue = cookieEntry.trim().split('=');

                    if(keyValue.length >= 2 && keyValue[0] == 'cartToken') cartToken = keyValue[1];
        ...
        , _setupBaseData: function(base, request, original) {
            ...
            // set cartToken
            if(typeof request.setCartToken === 'function') request.setCartToken(this._getCartToken(original));
        ...
        , _setupRelatedTo: function(request, segments){
            ...
            if(segments.length > 1){ //event/5/venue/6
                var related = segments[len-2];
